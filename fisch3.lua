--[[
    Project: Fisch Hub (Working Auto-Fish & Instant Catch Edition)
    Author: MrDon
]]
...
createToggle("Auto Fish & Shake", "AutoFish")
...
-- Engine loops for Fly, Auto-Shake, and Instant Catch
local BV, BG
RunService.RenderStepped:Connect(function()
    pcall(function()
        ...
        if playerGui then
            -- Auto Shake UI clicking handler
                    ...
                    if safeZone then
                        for _, btn in ipairs(safeZone:GetChildren()) do
                            if btn:IsA("ImageButton") or btn:IsA("TextButton") then
                                -- Fire virtual click event directly on mobile UI element
            ...
            -- Instant Catch handler for minigame/reel bar
            if Config.InstantCatch then
                local reelGui = playerGui:FindFirstChild("reel") or playerGui:FindFirstChild("Minigame")
                if reelGui then
                    -- Automatically complete progress values if found in controllers
                    for _, obj in ipairs(reelGui:GetDescendants()) do
                        if obj.Name == "playerbar" or obj.Name == "bar" then
                            if obj:IsA("GuiObject") then
                                obj.Position = UDim2.new(0, 0, obj.Position.Y.Scale, obj.Position.Y.Offset)
                            end
                        end
                    end
                end
                
                -- Fire backend remote events if available to instantly finish catching
                pcall(function()
                    for _, remote in ipairs(ReplicatedStorage:GetDescendants()) do
                        if remote:IsA("RemoteEvent") and (remote.Name:lower()..string.lower(remote.Parent.Name)):find("catch") then
                            remote:FireServer()
                        end
                    end
                end)
