--[[
    Project: Fisch Hub (Ultimate All-in-One Edition)
    Author: MrDon
]]
...
createToggle("Auto Shake", "AutoFish")
createToggle("Instant Catch", "InstantCatch")
createToggle("Bar Follow Fish", "BarFollow")
...
local islands = {
    ...
    ["Sunstone Island"] = Vector3.new(-870, 135, -1100),
    ...
    ["Snowcap Island"] = Vector3.new(2625, 135, 2370)
}
...
-- Engine loops for Fly, Auto-Shake, Instant Catch, and Bar Following
local BV, BG
RunService.RenderStepped:Connect(function()
    pcall(function()
        ...
        if playerGui then
            -- 1. Auto Shake Handler
            if Config.AutoFish then
                local shakeGui = playerGui:FindFirstChild("shake")
                if shakeGui then
            ...
            -- 2. Instant Catch Handler
            if Config.InstantCatch then
                local minigame = playerGui:FindFirstChild("reel") or playerGui:FindFirstChild("Minigame")
                if minigame then
                    for _, obj in ipairs(minigame:GetDescendants()) do
                        if obj:IsA("NumberValue") or obj:IsA("IntValue") then
                            if obj.Name:lower():find("progress") or obj.Name:lower():find("value") then
                                obj.Value = 100
                            end
                        end
                    end
            ...
            -- 3. Bar Follow Fish Handler
            if Config.BarFollow then
                local reelGui = playerGui:FindFirstChild("reel") or playerGui:FindFirstChild("Minigame")
                if reelGui then
                    local container = reelGui:FindFirstChild("bar") or reelGui:FindFirstChild("Container")
                    if container then
                        local fish = container:FindFirstChild("fish")
                        local marker = container:FindFirstChild("playerbar") or container:FindFirstChild("bar")
                        
                        if fish and marker and fish:IsA("GuiObject") and marker:IsA("GuiObject") then
                            marker.Position = UDim2.new(fish.Position.X.Scale, fish.Position.X.Offset, marker.Position.Y.Scale, marker.Position.Y.Offset)
                        end
                    end
                end
            end
